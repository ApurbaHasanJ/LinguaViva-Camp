...
const ManageClasses = () => {
  ...
  const { data: classes = [], refetch, isLoading } = useQuery(["classes"], async () => {
    const res = await fetch("https://b7a12-summer-camp-server-side-apurba-hasan-j.vercel.app/classes");
    return res.json();
  });
  ...
  const updateClassStatus = useMutation(
    async ({ classId, status, feedback }) => {
      const res = await fetch(`https://b7a12-summer-camp-server-side-apurba-hasan-j.vercel.app/classes/${classId}/status`, {
        method: "PATCH",
        headers: {
